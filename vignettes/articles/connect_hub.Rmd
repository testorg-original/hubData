...
Other hubverse tools, designed for data validation, [ensemble building](https://testorg-original.github.io/hubEnsembles/), [visualization](https://github.com/testorg-original/hubVis), etc..., all are designed with the "promises" implicit in the data format specified by `model_out_tbl`. For example, [the `hubEnsembles::linear_pool()` function ](https://testorg-original.github.io/hubEnsembles/reference/linear_pool.html) both accepts as input and returns as output `model_out_tbl` objects.
